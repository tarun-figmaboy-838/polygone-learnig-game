...
39 screens. If a line of copy needs to change, it changes here and nowhere
...
| `screens` card simulation | Instruction card matches the deck on all 39 screens |
...
| `node playthrough.js` | A scripted child plays all 39 screens, tries a wrong answer first on every judged one, asserts one wrong cue per wrong attempt |
